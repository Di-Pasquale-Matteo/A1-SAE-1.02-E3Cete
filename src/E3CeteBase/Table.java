...
public class Table {
    ...
    public boolean carteExiste(Coordonnees coordonnees) {
        return coordonnees.getLigne() >= 0 && coordonnees.getLigne() < hauteur && coordonnees.getColonne() >= 0 && coordonnees.getColonne() < largeur;
    }
    ...
    public int faireSelectionneUneCarte() {
        int numeroCarte = -1;
        while (numeroCarte == -1) {
            System.out.println("Entrez les coordonnées (ex. 1,1):");
    ...
    public void afficherSelection(int[] selection) {
        ...
        for (int i = 0; i < selection.length; i++) {
            ...
            if (tableau[ligne][colonne] != null) {
                resultat += tableau[ligne][colonne].toString() + " ";
            } else {
                resultat += "[Vide] ";
            }
        }
